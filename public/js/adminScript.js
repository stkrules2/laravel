            ...
            success: function(response) {
                ...
                if (response.length != 0) {
                    ...
                    $.each(response, function(key, value) {
                        $(".edit-dish-list").append(
                            " <li><span>" +
                                response[key].name +
                                '</span><span><button class="btn btn-light edit-dish-btn" id="edit-dish-btn-' +
                                response[key].id +
                                '" data-toggle="modal" data-target="#editDish"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button><a href="dish/delete/' +
                                response[key].id +
                                '"><button class="btn btn-danger"><i class="fa fa-trash-o" aria-hidden="true"></i></button></a>'
                        );
                    });
                } else {
                    $(".edit-dish-list").empty();
    ...
    $(".edit-dish-list").on("click", ".edit-dish-btn", function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('input[name="edit-dish-name"]').attr(
                    "value",
                    response[0].name
                );
                $('input[name="edit-dish-price"]').attr(
                    "value",
                    response[0].price
                );
                $('input[name="edit-dish-discount"]').attr(
                    "value",
                    response[0].before_discount_price
                );
                $('[name="edit-dish-description"]').val(
                ...
                $(
                    "<img src='../storage/" +
                        response[0].image1 +
                        "' class='delete-img-class'></img>"
                ).insertAfter(".edit-dish-image1");
                $(".edit-dish-image1")
                    .parent()
                    .prepend(
                ...
                if (response[0].image2 != null) {
                    $(
                        "<img src='../storage/" +
                            response[0].image2 +
                            "' class='delete-img-class'></img>"
                    ).insertAfter(".edit-dish-image2");
                    $(".edit-dish-image2")
                        .parent()
                        .prepend(
                            '<button type="button" id="delete-img-btn" class="dish-2" name="' +
                                response[0].id +
                ...
                if (response[0].image3 != null) {
                    $(
                        "<img src='../storage/" +
                            response[0].image3 +
                            "' class='delete-img-class'></img>"
                    ).insertAfter(".edit-dish-image3");
                    $(".edit-dish-image3")
                        .parent()
                        .prepend(
                            '<button type="button" id="delete-img-btn" class="dish-3" name="' +
                                response[0].id +
                                '"><i class="fa fa-times" aria-hidden="true"></i></button>'
                        );
                }
